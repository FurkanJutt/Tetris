...
public class Tetris_GameController : MonoBehaviour
{
    ...
    #region CUSTOM_MOVEMENT_FUNCTIONS
    public Transform currentTetromino;

    // Function to move the Tetromino to the right
    public void MoveRight()
    {
        currentTetromino.position += Vector3.right;
    ...
    // Function to move the Tetromino to the left
    public void MoveLeft()
    {
        currentTetromino.position += Vector3.left;
        if (!ValidMove(currentTetromino))
        {
            currentTetromino.position -= Vector3.left;
        }
    ...
    // Function to instantly drop the Tetromino
    public void InstantDrop()
    {
        currentTetromino.position += Vector3.down;
        while (!ValidMove(currentTetromino))
        {
            currentTetromino.position -= Vector3.down;
        }
    }
    ...
    public void MoveDown()
    {
        ...
        {
            // Handle Tetromino locking, line clearing, and spawning a new Tetromino
            //LockTetromino();
            //ClearLines();
            //DeleteLine();
            //SpawnNextTetromino();
    ...
    // Function to check if the Tetromino can move in a given direction
    private bool CanMove(Vector3 direction)
    {
        foreach (Transform block in currentTetromino)
        {
            Vector3 newPosition = block.position + direction;

            // Check if newPosition is within the grid boundaries
            if (!IsInsideGrid(newPosition))
            {
                Debug.Log($"{GetType().Name}-> Can't Move!");
                return false;
            }

            // Check if newPosition collides with existing blocks in the grid
            if (grid[(int)newPosition.x, (int)newPosition.y] != null)
            {
                Debug.Log($"{GetType().Name}-> Can't Move!");
                return false;   
            }
        }
        Debug.Log($"{GetType().Name}-> Can Move!");
        return true;
    }

    // Function to check if a position is inside the grid boundaries
    private bool IsInsideGrid(Vector3 position)
    {
        ...
        return (int)roundedX >= Tetris_Helper.WIDTH && (int)roundedX < 0 && (int)roundedY >= Tetris_Helper.HEIGHT;
    }

    #endregion CUSTOM_MOVEMENT_FUNCTIONS
    ...
    private void CreateDeadBlock()
    {
        foreach (Transform children in currBlock.transform)
        {
            ...
            Sprite currColor = children.GetComponent<SpriteRenderer>().sprite;
            if (grid[roundedY, roundedX] == null)
            {
                ...
                curr.sprite.GetComponent<SpriteRenderer>().sprite = currColor;
    ...
    private void NewBlock()
    {
        ...
        currBlock = Instantiate(Blocks[nextBlock], startPos, Quaternion.identity, blockParent);
        NewGhost();
        NextBlock();
        isShowingAnimation = false;
        if (grid[18, 4] != null)
        ...
        //print("newblock end");
        currentTetromino = currBlock.transform;
    }
    ...
    private void NewGhost()
    {
        if (ghostBlock != null)
        {
            Destroy(ghostBlock);
            //ghostBlock.Destroy();
            ghostBlock = Instantiate(Ghosts[nextBlock], currBlock.transform.position, Quaternion.identity, ghostParent);
        }
        else
        {
            ghostBlock = Instantiate(Ghosts[nextBlock], currBlock.transform.position, Quaternion.identity, ghostParent);
